...
/****** Object:  View [qc].[SPP_11_A]    Script Date: 25/05/2026 08:40:07 ******/
SET ANSI_NULLS ON
GO
...
-- QC rule name: SPP.11.A Vocabulary - [equipment]
...
CASE WHEN "equipment" = '' THEN NULL ELSE "equipment" END as "equipment" 
FROM reporting."samplingprocess" ) 

, missing_codes AS ( 
SELECT /*sp.record_id,*/ sp.countrycode, sp.assessmentmethodid, sp."measurementtype", sp."equipment" 
FROM CTE_samplingprocess sp 
LEFT JOIN reference."vocabulary" v 
ON sp."equipment" = v."notation"  COLLATE Latin1_General_CI_AS AND v."vocabulary" = 'samplingequipment'
WHERE 
  (v."notation" IS NULL AND sp."equipment" IS NOT NULL) 
  OR
